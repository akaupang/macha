...
tf_base_dir="/home/manny/Documents/Work/UiO/Modeling/wien/scripts/bash/macha/transformato_prep_sandbox"
tf_apop_dir_name="apo_protein"
tf_ligs_dir_name="ligands"
s1inp_template="step1_pdbreader.template.inp"

tf_apop_dir=$(realpath "${tf_base_dir}/${tf_apop_dir_name}")
tf_ligs_dir=$(realpath "${tf_base_dir}/${tf_ligs_dir_name}")
...
run_t_submenu () {
  ...
  echo ""
  echo "Key"
  echo " 1 : Make toppar folders for ligands with CGenFF"
  echo " 2 : Create folder structures for Transformato"
  echo " 3 : Option3"
  echo " 4 : Option4"
  echo ""
...
################################################################################
  ...
  if [ "$submenuoption" = "1" ]; then
    ...
    # Create a template file for step1_pdbreader.inp

    s1inp="* TEMPLATE FOR STEP1 PDBREADER, MODIFIED FROM AN INPUT FILE                          \n"
    s1inp+="* GENERATED BY CHARMM-GUI (http://www.charmm-gui.org) v3.7 on Jun, 28. 2022.         \n"
    s1inp+="* READ PDB, MANIPULATE STRUCTURE IF NEEDED, AND GENERATE TOPOLOGY FILE               \n"
    s1inp+="*                                                                                    \n"
    s1inp+="                                                                                     \n"
    s1inp+="DIMENS CHSIZE 5000000 MAXRES 3000000                                                 \n"
    s1inp+="                                                                                     \n"
    s1inp+="! Read topology and parameter files                                                  \n"
    s1inp+="stream toppar.str                                                                    \n"
    s1inp+="                                                                                     \n"
    s1inp+="!Print heavy atoms with unknown coordinates                                          \n"
    s1inp+="coor print sele ( .not. INIT ) .and. ( .not. hydrogen ) end                          \n"
    s1inp+="                                                                                     \n"
    s1inp+="define ROT sele .byres. (.not. INIT .and. (type CB .or. type CG*)) .and. -           \n"
    s1inp+="                         .not. ( resn *PRO .or. resn *ALA ) end                      \n"
    s1inp+="define XXX sele ROT .and. type CA end                                                \n"
    s1inp+="set nres = ?nsel                                                                     \n"
    s1inp+="set ires = 1                                                                         \n"
    s1inp+="label build_rot                                                                      \n"
    s1inp+="    define YYY sele .byres. ( XXX .subset. @ires ) end                               \n"
    s1inp+="    define ZZZ sele .bonded. YYY .and. .not. YYY .and. .not. (type N .or. type C) end\n"
    s1inp+="    if ?nsel .gt. 0 define ROT sele ROT .and. .not. YYY end                          \n"
    s1inp+="    incr ires by 1                                                                   \n"
    s1inp+="if ires .le. @nres goto build_rot                                                    \n"
    s1inp+="                                                                                     \n"
    s1inp+="ic param                                                                             \n"
    s1inp+="ic build                                                                             \n"
    s1inp+="define LONEPAIR sele chem LPH end                                                    \n"
    s1inp+="if ?nsel .gt. 0 coor shake sele LONEPAIR end                                         \n"
    s1inp+="prnlev 0                                                                             \n"
    s1inp+="hbuild sele hydr end                                                                 \n"
    s1inp+="prnlev 5                                                                             \n"
    s1inp+="                                                                                     \n"
    s1inp+="! check if there are unknown coordinate                                              \n"
    s1inp+="define XXX sele .not. INIT show end                                                  \n"
    s1inp+="if ?nsel .gt. 0 stop ! ABNORMAL TERMINATION: Undefined coordinates                   \n"
    s1inp+="                                                                                     \n"
    s1inp+="ENERGY                                                                               \n"
    s1inp+="                                                                                     \n"
    s1inp+="define XXX sele ROT .and. type CB end                                                \n"
    s1inp+="if ?nsel .gt. 0 then                                                                 \n"
    s1inp+="   cons fix sele .not. ROT end                                                       \n"
    s1inp+="   MOVE ADD MVTP TORS DMAX 360.0 sele ROT .and. type CA end -                        \n"
    s1inp+="                                 sele ROT .and. type CB end                          \n"
    s1inp+="   MC NSTEP 5000                                                                     \n"
    s1inp+="   MOVE DELETE MVIN 1                                                                \n"
    s1inp+="   cons fix sele none end                                                            \n"
    s1inp+="endif                                                                                \n"
    s1inp+="                                                                                     \n"
    s1inp+="                                                                                     \n"
    s1inp+="open write unit 10 card name step1_pdbreader.psf                                     \n"
    s1inp+="write psf  unit 10 card                                                              \n"
    s1inp+="                                                                                     \n"
    s1inp+="open write card unit 10 name step1_pdbreader.crd                                     \n"
    s1inp+="write coor unit 10 card                                                              \n"
    s1inp+="                                                                                     \n"
    s1inp+="open write card unit 10 name step1_pdbreader.pdb                                     \n"
    s1inp+="write coor pdb  unit 10 official                                                     \n"
    s1inp+="                                                                                     \n"
    s1inp+="coor stat sele all end                                                               \n"
    s1inp+="                                                                                     \n"
    s1inp+="bomblev -1                                                                           \n"
    s1inp+="                                                                                     \n"
    s1inp+="coor orient                                                                          \n"
    s1inp+="coor stat sele all end                                                               \n"
    s1inp+="                                                                                     \n"
    s1inp+="! Molecular Volume                                                                   \n"
    s1inp+="calc dcel = 0.5                                                                      \n"
    s1inp+="calc xdim = int ( ( ?xmax - ?xmin + 5.0 ) / @dcel ) + 1                              \n"
    s1inp+="calc ydim = int ( ( ?ymax - ?ymin + 5.0 ) / @dcel ) + 1                              \n"
    s1inp+="calc zdim = int ( ( ?zmax - ?zmin + 5.0 ) / @dcel ) + 1                              \n"
    s1inp+="calc space = @xdim * @ydim * @zdim                                                   \n"
    s1inp+="                                                                                     \n"
    s1inp+="scalar wmain = radius                                                                \n"
    s1inp+="scalar wmain add 1.4    ! use solvent accessible surface for molecular volume        \n"
    s1inp+="scalar 1 = wmain                                                                     \n"
    s1inp+="scalar 2 set 6                                                                       \n"
    s1inp+="coor volume hole space @space sele .not. resname TIP3 end                            \n"
    s1inp+="set molvol = ?volume                                                                 \n"
    s1inp+="                                                                                     \n"
    s1inp+="calc cgtot = int ( ?cgtot )                                                          \n"
    s1inp+="                                                                                     \n"
    s1inp+="open write unit 90 card name step1_pdbreader.str                                     \n"
    s1inp+="write title unit 90                                                                  \n"
    s1inp+="* set ncharge = @cgtot                                                               \n"
    s1inp+="* set xmax = ?xmax                                                                   \n"
    s1inp+="* set ymax = ?ymax                                                                   \n"
    s1inp+="* set zmax = ?zmax                                                                   \n"
    s1inp+="* set xmin = ?xmin                                                                   \n"
    s1inp+="* set ymin = ?ymin                                                                   \n"
    s1inp+="* set zmin = ?zmin                                                                   \n"
    s1inp+="* set molvol = @molvol                                                               \n"
    s1inp+="*                                                                                    \n"
    s1inp+="                                                                                     \n"
    s1inp+="stop                                                                                 \n"
    
    echo ""
    if [ -f "${s1inp_template}" ]; then
      echo ""
      echo "A template for step1_pdbreader.inp: ${s1inp_template} was found. Using this file."
    else
      printf "%b" "${s1inp}" > "${s1inp_template}"
      echo ""
      echo "A template file for step1_pdbreader.inp; ${s1inp_template} was written"
      echo "based on the standard/built-in scheme."
      echo "This file can be modified and as long as it is present,"
      echo "it will take preference if this process option is selected again."
    fi

    cmps_loc="stream toppar.str                                                                    "

    cmps_repl="stream toppar.str                                                                    \n"
    cmps_repl+="hei                                                                                  \n"

    `sed -e "s/${cmps_loc}/${cmps_repl}/g" ${s1inp_template} > "test.out"`
    ...
    # copy apo protein system and create waterbox and complex dirs
    # copy ligand toppar folder from ligands/ to each 
    # modify input files - python?
